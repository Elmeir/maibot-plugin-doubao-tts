...
## v1.9.0 (2026-09-19) 新增：「语音后结束本轮」开关

1. **主页新增「语音后结束本轮」开关**（`[plugin] stop_planner_after_voice`，默认**开**）：
   - 开（默认，行为与 1.8.0 一致）：麦麦自主语音发送成功即结束本轮 planner，
     LLM 不会再调 reply 把同样的内容发一遍文字；失败路径若插件刚给用户发过失败提示
     （30 秒去重窗口内），同样结束本轮，避免提示 + reply 重复。
   - 关：语音照发，但交给麦麦照常继续（可能再补一条文字回复）。
   - 「工具语音内容来源=回复生成」模式**不受此开关影响**：该模式必须让 planner 继续调用
     reply 生成内容，再由发送钩子把回复整条转语音。
2. 纯新增配置字段，config_version 不变（宿主自动补默认值，旧配置无需改动）。
   self_test 新增 2 项断言（开关关闭时，成功/失败路径都不带 `stop_after_execution`）。
